...
- **WebSocket**: clients connect to `/ws` (axum + tokio-tungstenite). The server broadcasts two message types — `leaderboard_delta` (top-100 diffs, batched ~1/sec) and `pulse` (per-event globe tremors, throttled to a sampling rate when event volume spikes). Raw event firehose is never exposed.
- **Persistence (Postgres via `sqlx`)** — three tables.
  - `user_location (login, city, country, lat, lng, resolved_at)` — permanent geocoding cache.
  - `repo_metadata (full_name, description, language, author, topics, total_stars, fetched_at)` — 24h TTL enrichment cache.
...
13. **No raw event persistence.** The counter lives in the Rust process. Raw `WatchEvent`s are ephemeral. If the process restarts mid-day, today's counts rebuild from the next poll cycle onward. Acceptable loss.
